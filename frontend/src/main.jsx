...
function App() {
  ...
  const [evidence, setEvidence] = useState({ url: 'https://status.example.com/incidents/demo', digest: 'sha256:demo-incident-001' });
  ...
  return <div className="app">
    <header><div><span className="eyebrow">SUBSCRIPTION SLA ESCROW</span><h1>Pay for uptime you can verify.</h1><p className="lede">Prepaid GEN is settled against public incident evidence and on-chain SLA rules.</p></div><div className="network"><span className="dot"/> Studionet <button onClick={async () => { try { const result = await connectWallet(); setClient(result.client); setWallet(result.account); setMessage('Wallet connected.'); } catch (error) { setMessage(error.message); } }}>{wallet ? `${wallet.slice(0, 8)}…` : 'Connect wallet'}</button></div></header>
